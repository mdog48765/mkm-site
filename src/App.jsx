...
function parseYMD(s) {
  // safe parse YYYY-MM-DD, returns Date at local midnight; null if invalid/missing
...
/* ===== Upcoming & Past Shows (Pizza Records) =====
   - You edit src/shows.json with plain strings for date/time:
       { "dateText": "Sat, Nov 9", "timeText": "Doors 7 PM • Music 8 PM", "sortDate": "2025-11-09", ... }
   - sortDate is optional BUT recommended so we can auto-sort and move past shows.
*/
function UpcomingShows() {
  const today = startOfToday();

  const shows = Array.isArray(SHOWS) ? SHOWS : [];
  const withSort = shows.map((e) => ({ ...e, _sort: parseYMD(e.sortDate) }));

  const upcoming = withSort
    .filter((e) => (e._sort ? e._sort >= today : true)) // if no sortDate, treat as upcoming
    .sort((a, b) => {
      const da = a._sort ? a._sort.getTime() : Number.POSITIVE_INFINITY;
      const db = b._sort ? b._sort.getTime() : Number.POSITIVE_INFINITY;
      return da - db;
    });
          ...
          {upcoming.map((e) => (
            ...
            >
              {e.flyer ? (
                ...
                >
                  {/* NO CROP: object-contain, natural height */}
                  <img
                    src={e.flyer}
                    alt={`${e.title} flyer`}
...
function PastShows() {
  ...
  const past = withSort
    .filter((e) => (e._sort ? e._sort < today : false)) // needs a sortDate to be auto-moved to past
    .sort((a, b) => {
      // newest past first
      const da = a._sort ? a._sort.getTime() : 0;
      const db = b._sort ? b._sort.getTime() : 0;
      return db - da;
    })
