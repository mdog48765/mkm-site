...
export default function App() {
  ...
  const [messagePreviewLen, setMessagePreviewLen] = useState(0); // live count (uncontrolled textarea)
...
function SafeImg({ jpg, webp, alt }) {
  ...
  function validateForm(form) {
    ...
    if (bookingType === "External" && addOns.length > 12) {
      // sanity guard; should never happen
      errs.addOns = "Too many add-ons selected.";
    }
    // Optional: basic date presence (not strictly required)
  ...
  async function handleSubmit(e) {
    ...
    // Preview count for UX (not required)
    ...
    const chosenService =
      selectedService ||
      form.get("service") ||
      (isPizzaBooking ? "Pizza Records – Basic" : "External – Compact PA");
